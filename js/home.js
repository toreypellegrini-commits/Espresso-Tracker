...
function renderOpenBags() {
  ...
  const bagMeta = activeBags.map(r => {
    ...
    // Most recent shot for this bag (shots are stored newest-first; still scan to be safe)
    let lastShotTs = 0;
    for (const s of shots) {
      if (s.roastLibId == r.id) {
        const t = new Date(s.date).getTime();
        if (t > lastShotTs) lastShotTs = t;
      }
    }
  ...
  container.innerHTML = bagMeta.map(({ r, days, phase }) => {
    ...
    // Show reference recipe if set, otherwise most recent shot
    const referenceShot = r.referenceShotId ? shots.find(s => s.id == r.referenceShotId) : null;
    const lastShot = shots.find(s => s.roastLibId == r.id);
    const displayShot = referenceShot || lastShot;
    const displayLabel = referenceShot ? 'Ref' : 'Last';
    let lastHTML = '';
    if (displayShot) {
      const parts = [];
      if (displayShot.dose) parts.push(`<span>${displayShot.dose}g in</span>`);
      if (displayShot.yield) parts.push(`<span>${displayShot.yield}g out</span>`);
      if (displayShot.time) parts.push(`<span>${displayShot.time}s</span>`);
      if (displayShot.grind) parts.push(`<span>grind ${displayShot.grind}</span>`);
      if (!referenceShot && displayShot.rating) parts.push(`<span>${'★'.repeat(displayShot.rating)}</span>`);
      if (parts.length) {
        lastHTML = `<div class="bag-card-last">${displayLabel}: ${parts.join(' · ')}</div>`;
      }
    }
