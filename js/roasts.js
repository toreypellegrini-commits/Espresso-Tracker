...
function renderLibCard(r){
  ...
  let dc='';
  if(days!==null&&!r.finished){const p=getRoastPhase(days,r.restDays);if(p)dc=`<span class="days-badge ${p.cls}">${days}d \u00b7 ${p.label}</span>`;}
...
function openEditRoast(id){
  ...
  setField('m-roaster',r.roaster);setField('m-roastname',r.roastName||'');setField('m-origin',r.origin);setField('m-varietal',r.varietal||'');setField('m-desc',r.desc||'');setField('m-roastdate',r.roastDate||'');setField('m-restdays',r.restDays!=null?r.restDays:'');
